...
func TestBasicFunc(t *testing.T) {
	...
	block, err := ledger.FormatRootBlock([]*protos.Transaction{t1})
	if err != nil {
	...
	if readErr != nil {
		...
		t.Logf("block detail: %v", blockCopy)
	}
	...
	if readErr2 != nil {
		...
		t.Logf("block2 detail: %v", blockCopy2)
	}
...
func TestSplitFunc(t *testing.T) {
	...
	block, err := ledger.FormatBlock([]*protos.Transaction{t1, t2},
		[]byte("xchain-Miner-1"),
		ecdsaPk,
		123456789,
		0,
	...
	block4, err := ledger.FormatBlock([]*protos.Transaction{t1, t2},
	...
	if findErr != nil {
		t.Fatal("fail to to find base parent of two blocks", "destBlockid",
...
func TestTruncate(t *testing.T) {
	...
	block1, err := ledger.FormatRootBlock([]*protos.Transaction{t1})
	if err != nil {
		t.Fatalf("format block fail, %v", err)
	}
	...
	//block2 <- block3
	block3, err := ledger.FormatBlock([]*protos.Transaction{&protos.Transaction{Txid: []byte("dummy1")}},
	...
	//block2 <- block4
	block4, err := ledger.FormatBlock([]*protos.Transaction{&protos.Transaction{Txid: []byte("dummy2")}},
