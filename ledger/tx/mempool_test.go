...
func TestPutOrphanTx(t *testing.T) {
	...
	err := m.PutTx(tx) // 添加孤儿交易，mempool 应该生成一个 mock node 以及把当前交易加入孤儿列表。
	if err != nil {
		t.Fatal(err)
	}
	if len(m.orphans) != 2 {
	...
	err = m.PutTx(tx1) // 添加上一个孤儿交易的所依赖的交易，也就是已经在 mempool 中的 mock node。orphans 中应该有三个节点。
	if err != nil {
		t.Fatal(err)
	}
	if len(m.orphans) != 3 {
...
func NewTxForTest(txid []byte, txInputs []*protos.TxInput, txOutput []*protos.TxOutput,
	txInputsExt []*protos.TxInputExt, txOutputsExt []*protos.TxOutputExt) *protos.Transaction {
...
func run(b *testing.B, t *testing.T) {
	...
	// mlock.DeleteTxAndChildren(deleteID)
	// mlock.ConfirmeTx(result[800])
	// if e != nil {
	// 	panic(e)
	// }
	printMempool(m)
	// _, ok := mlock.unconfirmed[deleteID]
...
// var result []*protos.Transaction

// func ff(tx *protos.Transaction) bool {
// 	result = append(result, tx)
// 	return true
// }

func batchTx(m *Mempool) []*protos.Transaction {
	now := time.Now()
	var rrr []*protos.Transaction
	m.Range(func(tx *protos.Transaction) bool {
		rrr = append(rrr, tx)
		return true
	})
	end := time.Now()
