...
func MakeMerkleTree(txList []*protos.Transaction) [][]byte {
	txCount := len(txList)
	if txCount == 0 {
		return nil
	}
...
// func FastMakeMerkleTree(txList []*protos.Transaction) [][]byte {
// 	txCount := len(txList)
// 	if txCount == 0 {
// 		return nil
// 	}
...
//序列化系统合约失败的Txs
func encodeFailedTxs(buf *bytes.Buffer, block *protos.InternalBlock) error {
	txids := []string{}
...
func encodeJustify(buf *bytes.Buffer, block *protos.InternalBlock) error {
...
// VerifyMerkle
func VerifyMerkle(block *protos.InternalBlock) error {
...
func MakeBlockID(block *protos.InternalBlock) ([]byte, error) {
