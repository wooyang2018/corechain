...
type Ledger struct {
	...
	blocksTable    storage.Database // 区块表
	mutex          *sync.RWMutex
	xlog           logger.Logger      //日志库
	meta           *protos.LedgerMeta //账本关键的元数据{genesis, tip, height}
	GenesisBlock   *GenesisBlock      //创始块
	pendingTable   storage.Database   //保存临时的block区块
	heightTable    storage.Database   //保存高度到Blockid的映射
	...
	confirmBatch   storage.Batch //新增区块
}
