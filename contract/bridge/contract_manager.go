...
func (c *contractManager) DeployContract(kctx contract.KContext) (*contract.Response, contract.Limits, error) {
	...
	if err := state.Put("contract", contract.ContractCodeDescKey(contractName), descbuf); err != nil {
		return nil, contract.Limits{}, err
	}
	if err := state.Put("contract", contract.ContractCodeKey(contractName), code); err != nil {
		return nil, contract.Limits{}, err
	...
	if desc.ContractType == string(TypeEvm) {
		...
		if err := state.Put("contract", contract.ContractAbiKey(contractName), abiBuf); err != nil {
			return nil, contract.Limits{}, err
		}
	}
...
func (c *contractManager) UpgradeContract(kctx contract.KContext) (*contract.Response, contract.Limits, error) {
	...
	store := kctx
	store.Put("contract", contract.ContractCodeDescKey(contractName), descbuf)
	store.Put("contract", contract.ContractCodeKey(contractName), code)
	...
	return &contract.Response{
			...
			Disk: contract.ModelCacheDiskUsed(store),
		}, nil
}
