...
func (c *contractProcess) waitReply() error {
	...
	defer cancel()
	for {
		_, err := c.rpcClient.Ping(ctx, new(protos.PingRequest))
		if err == nil {
			return nil
		}
		select {
...
func (c *contractProcess) resetRpcClient() error {
	...
	c.rpcPort = port
	c.rpcConn = conn
	c.rpcClient = protos.NewNativeCodeClient(c.rpcConn)
	return nil
}

func (c *contractProcess) RpcClient() protos.NativeCodeClient {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	return c.rpcClient
}
