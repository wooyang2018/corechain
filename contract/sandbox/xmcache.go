...
func (xc *XMCache) flushUTXORWSet() error {
	...
	var err error
	if len(inputs) != 0 {
		in, err = model.MarshalMessages(inputs)
		if err != nil {
			return err
		}
	}
	if len(outputs) != 0 {
		out, err = model.MarshalMessages(outputs)
		if err != nil {
			return err
		}
	}
...
func ParseContractEvents(tx *protos.Transaction) ([]*protos.ContractEvent, error) {
	...
	for _, out := range tx.GetTxOutputsExt() {
		...
		err := model.UnmsarshalMessages(out.GetValue(), &events)
		if err != nil {
			return nil, err
		}
		break
...
func (xc *XMCache) writeEventRWSet() error {
	...
	buf, err := model.MarshalMessages(xc.events)
	if err != nil {
		return err
	}
