...
func (m *MemXModel) Select(bucket string, startKey []byte, endKey []byte) (ledger.XIterator, error) {
	if bucket == "" {
...
func (m *MemXModel) NewIterator() ledger.XIterator {
	return newTreeIterator(m.tree)
}

// treeIterator 把tree的Iterator转换成XIterator
...
func newTreeIterator(tree *redblacktree.Tree) ledger.XIterator {
	iter := tree.Iterator()
	return &treeIterator{
		cmp:  tree.Comparator,
		iter: &iter,
...
func newTreeRangeIterator(tree *redblacktree.Tree, start, end []byte) ledger.XIterator {
	it := &treeIterator{
		cmp: tree.Comparator,
		end: end,
	}
