...
type UtxoVM struct {
	...
	utxoTable         storage.Database           // utxo表
	...
	cryptoClient      cryptoBase.CryptoClient    // 加密实例
	...
	lastBatch         storage.Batch // 上一个交易对应的 batch，postTx 时每个交易的 batch 不同，但是执行区块时（walk 或者 play）batch 相同。
}
...
func (uv *UtxoVM) UnlockKey(utxoKey []byte) {
	uv.MutexMem.Lock()
	defer uv.MutexMem.Unlock()
	uv.log.Debug("unlock utxo key", "key", string(utxoKey))
...
func (uv *UtxoVM) tryLockKey(key []byte) bool {
	...
	if _, exist := uv.lockKeys[string(key)]; !exist {
		...
		uv.log.Debug("lock utxo key", "key", string(key))
		return true
	}
	return false
}
...
func NewUtxo(sctx *context.StateCtx, metaHandle *meta.Meta, stateDB storage.Database) (*UtxoVM, error) {
...
func (uv *UtxoVM) SelectUtxos(fromAddr string, totalNeed *big.Int, needLock, excludeUnconfirmed bool) ([]*protos.TxInput, [][]byte, *big.Int, error) {
	...
	if l2Cache, exist := uv.UtxoCache.Available[fromAddr]; exist {
		for uKey, uItem := range l2Cache {
			if uItem.FrozenHeight > curLedgerHeight || uItem.FrozenHeight == -1 {
				uv.log.Debug("utxo still frozen, skip it", "uKey", uKey, " fheight", uItem.FrozenHeight)
				continue
			}
	...
	if !foundEnough {
		...
		it := storage.NewQuickIterator(uv.ldb, []byte(addrPrefix), middleKey)
		defer it.Release()
		for it.Next() {
			...
			if uItem.FrozenHeight > curLedgerHeight || uItem.FrozenHeight == -1 {
				uv.log.Debug("utxo still frozen, skip it", "key", string(key), "fheight", uItem.FrozenHeight)
				continue
			}
...
func (uv *UtxoVM) RemoveUtxoCache(address string, utxoKey string) {
	uv.log.Debug("RemoveUtxoCache", "address", address, "utxoKey", utxoKey)
...
func (uv *UtxoVM) GetAccountContracts(account string) ([]string, error) {
	...
	for it.Next() {
		key := string(it.Key())
		ret := strings.Split(key, base.GetACLSeparator())
		size := len(ret)
		if size >= 1 {
...
func (uv *UtxoVM) QueryUtxoRecord(accountName string, displayCount int64) (*protos.UtxoRecordDetail, error) {
	...
	utxoRecordDetail.OpenUtxo = &protos.UtxoRecord{
		UtxoCount:  openUtxoCount.String(),
		UtxoAmount: openUtxoAmount.String(),
		Item:       openItem,
	}
	utxoRecordDetail.LockedUtxo = &protos.UtxoRecord{
		UtxoCount:  lockedUtxoCount.String(),
		UtxoAmount: lockedUtxoAmount.String(),
		Item:       lockedItem,
	}
	utxoRecordDetail.FrozenUtxo = &protos.UtxoRecord{
...
func (uv *UtxoVM) QueryAccountContainAK(address string) ([]string, error) {
	...
	for it.Next() {
		key := string(it.Key())
		ret := strings.Split(key, base.GetAKAccountSeparator())
		size := len(ret)
		if size >= 1 {
