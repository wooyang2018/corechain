...
func (t *State) verifySignatures(tx *protos.Transaction, digestHash []byte) (bool, map[string]bool, error) {
	...
	// verify initiator
	akType := base.IsAccount(tx.Initiator)
	if akType == 0 {
		// check initiator address signature
		ok, err := permission.IdentifyAK(tx.Initiator, tx.InitiatorSigns[0], digestHash)
		if err != nil || !ok {
		...
		for _, sign := range tx.InitiatorSigns {
			...
			ok, err := permission.IdentifyAK(addr, sign, digestHash)
			if !ok {
				t.log.Warn("verifySignatures failed", "address", tx.Initiator, "error", err)
				return ok, nil, err
			}
		...
		ok, err := permission.IdentifyAccount(t.sctx.AclMgr, tx.Initiator, initiatorAddr)
		if !ok {
	...
	for idx, authReq := range tx.AuthRequire {
		...
		ok, err := permission.IdentifyAK(addr, signInfo, digestHash)
		if err != nil || !ok {
...
func (t *State) verifyXuperSign(tx *protos.Transaction, digestHash []byte) (bool, map[string]bool, error) {
...
func (t *State) verifyUTXOPermission(tx *protos.Transaction, verifiedID map[string]bool) (bool, error) {
	...
	for _, txInput := range tx.TxInputs {
		...
		if akType == 1 {
			...
			if ok, err := permission.IdentifyAccount(t.sctx.AclMgr, string(name), tx.AuthRequire); !ok {
...
func (t *State) verifyContractOwnerPermission(contractName string, tx *protos.Transaction,
	verifiedID map[string]bool) (bool, error) {
	versionData, confirmed, err := t.xmodel.GetWithTxStatus(base.GetContract2AccountBucket(), []byte(contractName))
	...
	ok, err := permission.IdentifyAccount(t.sctx.AclMgr, accountName, tx.AuthRequire)
	if err == nil && ok {
...
func (t *State) verifyRWSetPermission(tx *protos.Transaction, verifiedID map[string]bool) (bool, error) {
	req := tx.GetContractRequests()
	// if not contract, pass directly
	if req == nil {
		return true, nil
	}
	writeSet := []*ledger.PureData{}
	for _, txOut := range tx.TxOutputsExt {
		writeSet = append(writeSet, &ledger.PureData{Bucket: txOut.Bucket, Key: txOut.Key, Value: txOut.Value})
	}
	for _, ele := range writeSet {
		...
		case base.GetContract2AccountBucket():
			...
			ok, accountErr := permission.IdentifyAccount(t.sctx.AclMgr, accountName, tx.AuthRequire)
			if !ok {
...
func (t *State) verifyContractPermission(tx *protos.Transaction, allUsers []string) (bool, error) {
	req := tx.GetContractRequests()
	if req == nil {
	...
	for i := 0; i < len(req); i++ {
		...
		ok, err := permission.CheckContractMethodPerm(t.sctx.AclMgr, allUsers, contractName, methodName)
		if err != nil || !ok {
...
func getGasLimitFromTx(tx *protos.Transaction) (int64, error) {
...
func (t *State) verifyContractTxAmount(tx *protos.Transaction) (bool, error) {
	amountOut := new(big.Int).SetInt64(0)
	req := tx.GetContractRequests()
	contractName, amountCon, err := ltx.ParseContractTransferRequest(req)
	if err != nil {
		return false, err
	}
...
func (t *State) verifyTxRWSets(tx *protos.Transaction) (bool, error) {
	...
	utxoInput, err := model.ParseContractUtxoInputs(tx)
	if err != nil {
		return false, err
	}
	...
	transContractName, transAmount, err := ltx.ParseContractTransferRequest(req)
	if err != nil {
		return false, err
	}
	...
	RWSet := sandBox.RWSet()
	t.log.Debug("verifyTxRWSets", "env.output", wset, "writeSet", RWSet.WSet)
	ok := model.Equal(wset, RWSet.WSet)
	if !ok {
...
func (t *State) verifyMarked(tx *protos.Transaction) (bool, bool, error) {
...
func (t *State) verifyMarkedTx(tx *protos.Transaction) error {
...
// err
func (t *State) verifyRelyOnMarkedTxs(tx *protos.Transaction) (bool, bool, error) {
	isRelyOnMarkedTx := false
...
func (t *State) VerifyContractPermission(initiator string, authRequire []string, contractName, methodName string) (bool, error) {
	...
	return permission.CheckContractMethodPerm(t.sctx.AclMgr, allUsers, contractName, methodName)
}
...
func (t *State) VerifyContractOwnerPermission(contractName string, authRequire []string) error {
	...
	ok, err := permission.IdentifyAccount(t.sctx.AclMgr, accountName, authRequire)
	if err != nil {
		return err
	}
	if !ok {
...
func (t *State) verifyBlockTxs(block *protos.InternalBlock, isRootTx bool, unconfirmToConfirm map[string]bool) error {
	var err error
	var once sync.Once
	wg := sync.WaitGroup{}
	dags := ltx.SplitToDags(block)
	for _, txs := range dags {
		wg.Add(1)
		go func(txs []*protos.Transaction) {
			defer wg.Done()
...
func (t *State) GenRWSetFromTx(tx *protos.Transaction) ([]*ledger.VersionedData, []*ledger.PureData, error) {
	...
	for _, txIn := range tx.TxInputsExt {
		...
		t.log.Debug("prepareEnv", "verData", verData, "txIn", txIn)
		if model.GetVersion(verData) != GetVersion(txIn) {
			err := fmt.Errorf("prepareEnv fail, key:%s, inputs version is not valid: %s != %s", string(verData.PureData.Key), model.GetVersion(verData), GetVersion(txIn))
			return nil, nil, err
		}
		inputs = append(inputs, verData)
	}
	for _, txOut := range tx.TxOutputsExt {
		outputs = append(outputs, &ledger.PureData{Bucket: txOut.Bucket, Key: txOut.Key, Value: txOut.Value})
	}
