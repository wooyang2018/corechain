...
func NewState(sctx *sctx.StateCtx) (*State, error) {
	...
	obj.ldb, err = leveldb.CreateKVInstance(kvParam) //获取DataBase实例
	if err != nil {
		return nil, fmt.Errorf("create state failed because create ldb error:%s", err)
	}

	obj.xmodel, err = model.NewXModel(sctx, obj.ldb) //获取DataBase之上的XModel实例
	if err != nil {
	...
	loadErr := obj.tx.LoadUnconfirmedTxFromDisk() //从disk还原unconfirm表到内存
	if loadErr != nil {
		return nil, loadErr
	}
...
func (t *State) GetContractStatus(contractName string) (*protos.ContractStatus, error) {
	...
	txid := verdata.GetRefTxid() //获取合约ID
