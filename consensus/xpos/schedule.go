...
// XPOSSchedule 实现了ProposerElectionInterface接口，接口定义了proposers操作
// XPOSSchedule 是tdpos的主要结构，其能通过合约调用来变更smr的候选人信息，并且向smr提供对应round的候选人信息
type XPOSSchedule struct {
	address string
	// 出块间隔, 单位为毫秒
	period int64
	// 每轮每个候选人最多出多少块
...
func NewSchedule(xconfig *xposConfig, log logger.Logger, ledger base.LedgerRely, startHeight int64) *XPOSSchedule {
	...
	if !base.AddressEqual(schedule.validators, refresh) && len(refresh) != 0 {
...
func (s *XPOSSchedule) minerScheduling(timestamp int64) (term int64, pos int64, blockPos int64) {
...
func (s *XPOSSchedule) getSnapshotKey(height int64, bucket string, key []byte) ([]byte, error) {
	if height <= 0 {
		return nil, ErrHeightTooLow
	}
	block, err := s.ledger.QueryBlockHeaderByHeight(height)
	if err != nil {
		s.log.Debug("xpos::getSnapshotKey::QueryBlockByHeight err.", "err", err)
		return nil, err
	}
	reader, err := s.ledger.CreateSnapshot(block.GetBlockid())
	if err != nil {
		s.log.Error("xpos::getSnapshotKey::CreateSnapshot err.", "err", err)
		return nil, err
	}
	versionData, err := reader.Get(bucket, key)
	if err != nil {
		s.log.Debug("xpos::getSnapshotKey::reader.Get err.", "err", err)
		return nil, err
	}
...
func (s *XPOSSchedule) GetLeader(round int64) string {
...
func (s *XPOSSchedule) calAddTime(round int64, tipHeight int64) int64 {
...
func (s *XPOSSchedule) GetValidators(round int64) []string {
	...
	if calErr != nil {
		s.log.Debug("xpos::GetValidators::CalculateProposers err", "err", calErr)
		return nil
	}
	return proposers
}
...
func (s *XPOSSchedule) UpdateProposers(height int64) bool {
	...
	if !base.AddressEqual(nextProposers, s.validators) {
		s.log.Debug("xpos::UpdateProposers", "origin", s.validators, "proposers", nextProposers, "height", height)
		s.validators = nextProposers
		return true
	}
	s.log.Debug("xpos::UpdateProposers", "origin", s.validators, "height", height)
	return false
}

// CalculateProposers 用于计算下一个Round候选人的值，仅在updateProposers和smr接口时使用
func (s *XPOSSchedule) CalculateProposers(height int64) ([]string, error) {
	if height < s.startHeight+3 {
	...
	if err != nil {
		s.log.Error("xpos::CalculateProposers::calculateTopK err.", "err", err)
		return nil, err
	}
	return p, nil
}
...
func (s *XPOSSchedule) calTopKNominator(height int64) ([]string, error) {
	if height < s.startHeight+3 {
		return s.initValidators, nil
	}
	// 获取nominate信息
	...
	if err != nil {
		s.log.Error("xpos::calculateTopK::getSnapshotKey err.", "err", err)
		return nil, err
	}
	// 未读到值时直接返回初始化值
	if res == nil {
	...
	if err := json.Unmarshal(res, &nominateValue); err != nil {
		s.log.Error("xpos::calculateTopK::load nominate read set err.")
		return nil, err
	}
	...
	for candidate, _ := range nominateValue {
		...
		if err != nil {
			s.log.Error("xpos::calculateTopK::load vote read set err.")
			return nil, err
		}
		// 未有vote信息则跳过
		if res == nil {
	...
	if int64(termBallotSli.Len()) < s.proposerNum {
		s.log.Debug("xpos::calculateTopK::Term publish proposer num less than config", "termVotes", termBallotSli)
		return s.initValidators, nil
	}
	// 计算topK候选人
...
func (s *XPOSSchedule) CalOldProposers(height int64, timestamp int64, storage []byte) ([]string, error) {
	if height < s.startHeight+3 {
		return s.initValidators, nil
	}
	...
	if s.enableChainedBFT && storage != nil {
		...
		justify, err := quorum.ParseOldQCStorage(storage)
		if err != nil {
			return nil, err
		}
		if justify.TargetBits != 0 {
			s.log.Debug("xpos::CalOldProposers::use rollback target.")
	...
	if err != nil {
		s.log.Error("xpos::CalOldProposers::calculateTopK err.", "err", err)
		return nil, err
	}
	s.log.Debug("xpos::CalOldProposers::calTopKNominator", "p", p, "target height", targetHeight)
	return p, nil
}

// calHisValidators 根据term计算历史候选人信息
func (s *XPOSSchedule) calHisValidators(height int64) ([]string, error) {
	...
	block, err := s.ledger.QueryBlockHeaderByHeight(height)
	if err != nil {
		s.log.Error("xpos::CalculateProposers::QueryBlockByHeight err.", "err", err)
		return nil, err
	}
	...
	s.log.Debug("xpos::CalculateProposers::target height.", "inputHeight", height, "targetHeight", targetHeight,
...
func (s *XPOSSchedule) binarySearch(begin int64, end int64, term int64) (int64, error) {
	for begin < end {
...
func (s *XPOSSchedule) getTerm(pos int64) (int64, error) {
	...
	storage, ok := in.(*quorum.ConsensusStorage)
	if !ok {
