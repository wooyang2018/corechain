...
func TestCalHisValidators(t *testing.T) {
	...
	l.Put(cmock.NewFakeBlock(7))
	l.Put(cmock.NewFakeBlock(8))
	l.Put(cmock.NewFakeBlock(9))
	l.Put(cmock.NewFakeBlock(10))
	l.Put(cmock.NewFakeBlock(11))
	// 2. 整理Block的共识存储
	...
	// 3. 构造nominate存储
	l.SetSnapshot(posBucket, []byte(nominateKey), NominateKey1())
	// 4. 构造vote存储
	l.SetSnapshot(posBucket, []byte(voteKeyPrefix+"TeyyPLpp9L7QAcxHangtcHTu7HUZ6iydY"), VoteKey1())
	l.SetSnapshot(posBucket, []byte(voteKeyPrefix+"SmJG3rH2ZzYQ9ojxhbRCPwFiE9y6pD1Co"), VoteKey2())
	l.SetSnapshot(posBucket, []byte(voteKeyPrefix+"akf7qunmeaqb51Wu418d6TyPKp4jdLdpV"), VoteKey3())
	// 5. 调用查看
	...
	if !base.AddressEqual(v1, []string{"TeyyPLpp9L7QAcxHangtcHTu7HUZ6iydY", "SmJG3rH2ZzYQ9ojxhbRCPwFiE9y6pD1Co"}) {
