...
func (tp *tdposConsensus) runNominateCandidate(contractCtx contract.KContext) (*contract.Response, error) {
	...
	if amount <= 0 || err != nil {
		return base.NewContractErrResponse(ErrAmount.Error()), ErrAmount
	}
	// 1.2 是否按照要求多签
	if ok := tp.isAuthAddress(candidateName, contractCtx.Initiator(), contractCtx.AuthRequire()); !ok {
		return base.NewContractErrResponse(ErrAuth.Error()), ErrAuth
	}
	// 1.3 调用冻结接口
	...
	if res != nil { // 非首次初始化
		if err := json.Unmarshal(res, &nominateValue); err != nil {
			tp.log.Error("xpos::runNominateCandidate::load read set err.")
			return base.NewContractErrResponse("Internal error."), err
		}
	}
	if _, ok := nominateValue[candidateName]; ok { // 已经提过名
		return base.NewContractErrResponse(ErrRepeatNominate.Error()), ErrRepeatNominate
	}
...
func (tp *tdposConsensus) runRevokeCandidate(contractCtx contract.KContext) (*contract.Response, error) {
	...
	if res != nil {
		if err := json.Unmarshal(res, &nominateValue); err != nil {
			tp.log.Error("xpos::runRevokeCandidate::load read set err.")
			return base.NewContractErrResponse(ErrValueNotFound.Error()), err
		}
	}
	// 1.1 查看是否有历史投票
	v, ok := nominateValue[candidateName]
	if !ok {
		return base.NewContractErrResponse(ErrEmptyNominateKey.Error()), ErrEmptyNominateKey
	}
	ballot, ok := v[contractCtx.Initiator()]
	if !ok {
		return base.NewContractErrResponse(ErrValueNotFound.Error()), ErrValueNotFound
	}
	...
	if res != nil {
		if err := json.Unmarshal(res, &revokeValue); err != nil {
			tp.log.Error("xpos::runRevokeCandidate::load revoke read set err.")
			return base.NewContractErrResponse(ErrValueNotFound.Error()), err
		}
	}
...
func (tp *tdposConsensus) runVote(contractCtx contract.KContext) (*contract.Response, error) {
	...
	nominateValue := NewNominateValue()
	if err := json.Unmarshal(res, &nominateValue); err != nil {
		tp.log.Error("xpos::runVote::load nominates read set err.")
		return base.NewContractErrResponse(err.Error()), err
	}
	if _, ok := nominateValue[candidateName]; !ok {
		return base.NewContractErrResponse(ErrVoteNominate.Error()), ErrVoteNominate
	}
	...
	if res != nil {
		if err := json.Unmarshal(res, &voteValue); err != nil {
			tp.log.Error("xpos::runVote::load vote read set err.")
			return base.NewContractErrResponse(err.Error()), err
		}
	}
...
func (tp *tdposConsensus) runRevokeVote(contractCtx contract.KContext) (*contract.Response, error) {
	...
	res, err := contractCtx.Get(tp.election.bindContractBucket, []byte(voteKey))
	if err != nil {
		tp.log.Error("xpos::runRevokeVote::load vote read set err when get key.")
		return base.NewContractErrResponse(err.Error()), err
	}
	voteValue := NewvoteValue()
	if err := json.Unmarshal(res, &voteValue); err != nil {
		tp.log.Error("xpos::runRevokeVote::load vote read set err.")
		return base.NewContractErrResponse(err.Error()), err
	}
	...
	if v < amount {
		return base.NewContractErrResponse("Your vote amount is less than have."), ErrEmptyNominateKey
	}
...
func (tp *tdposConsensus) runGetTdposInfos(contractCtx contract.KContext) (*contract.Response, error) {
	...
	res, err := contractCtx.Get(tp.election.bindContractBucket, []byte(nKey))
	if res == nil {
		return base.NewContractOKResponse([]byte("{}")), nil
	}
	if err != nil {
		return base.NewContractErrResponse("Internal error."), err
	}
	nominateValue := NewNominateValue()
	if err := json.Unmarshal(res, &nominateValue); err != nil {
		tp.election.log.Error("xpos: getTdposInfos: unmarshal nominate err.", "err", err)
		return base.NewContractErrResponse("Internal error."), err
	}
	...
	for candidate, _ := range nominateValue {
		...
		res, err = contractCtx.Get(tp.election.bindContractBucket, []byte(voteKey))
		if err != nil {
			tp.election.log.Error("xpos: getTdposInfos: load vote read set err when get key.", "key", voteKey)
			continue
		}
		voteValue := NewvoteValue()
		if res == nil {
		...
		if err := json.Unmarshal(res, &voteValue); err != nil {
			tp.election.log.Error("xpos: getTdposInfos: load vote read set err.", "res", res, "err", err)
			continue
		}
	...
	res, err = contractCtx.Get(tp.election.bindContractBucket, []byte(rKey))
	if err != nil && err.Error() != ErrNotFound.Error() {
		tp.election.log.Error("xpos: getTdposInfos: load revoke read set err when get key.")
		return base.NewContractErrResponse("Internal error."), err
	}
	revokeValue := NewRevokeValue()
	if res != nil {
