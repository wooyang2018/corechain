...
func GenerateAccountByMnemonic(mnemonic string, language int) (*ECDSAAccount, error) {
	...
	if cryptography != types.Nist {
		err = fmt.Errorf("Only cryptoGraphy NIST[%d] is supported in this version, this cryptoGraphy is [%v].", types.Nist, cryptography)
		return nil, err
	}
	curve := elliptic.P256()
...
func CreateNewAccountWithMnemonic(language int, strength uint8, cryptography uint8) (*ECDSAAccount, error) {
	...
	switch cryptography {
	case types.Nist: // NIST
		cryptographyBit = []byte{types.Nist}
	case types.Gm: // 国密
