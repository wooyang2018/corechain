...
func SplitPrivateKey(jsonPrivKey string, totalShareNumber, minimumShareNumber int) ([]string, error) {
	...
	for _, complexSecretMsg := range privateKeyByteGroup {
		complexShares, err := complexSecret.ComplexSecretSplit2(totalShareNumber, minimumShareNumber, complexSecretMsg, privateKey.Curve)
		if nil != err {
			return []string{}, err
		}
...
func RetrievePrivateKeyByShares(strEncodeComplexShareGroup []string) (string, error) {
	...
	for key := 1; true; key++ {
		...
		secretBytes, err := complexSecret.ComplexSecretRetrieve2(retrieveComplexShares, elliptic.P256())
		if nil != err {
			return "", err
		}
