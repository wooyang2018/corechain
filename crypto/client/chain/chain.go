...
import (
	...
	"github.com/wooyang2018/corechain/crypto/common/math/ecc"
	complexSecret "github.com/wooyang2018/corechain/crypto/common/secret/complex"
	...
	"github.com/wooyang2018/corechain/crypto/core/threshold/schnorr/dkg"
	"github.com/wooyang2018/corechain/crypto/core/threshold/schnorr/tss_sign"
	"github.com/wooyang2018/corechain/crypto/core/zkp/hash/mimc"
	walletBase "github.com/wooyang2018/corechain/crypto/hdwallet/base"
	"github.com/wooyang2018/corechain/crypto/hdwallet/key"
	walletRand "github.com/wooyang2018/corechain/crypto/hdwallet/rand"
)

func GetInstance() base.CryptoClient {
	gmCryptoClient := XchainCryptoClient{}
...
func (xcc *XchainCryptoClient) GetEcdsaPublicKeyJsonFormatStrFromPublicKey(k *ecdsa.PublicKey) (string, error) {
	jsonEcdsaPublicKeyJsonFormat, err := account.GetEcdsaPublicKeyJsonFormatFromPublicKey(k)
	return jsonEcdsaPublicKeyJsonFormat, err
}
...
func (xcc *XchainCryptoClient) CreateNewAccountWithMnemonic(language int, strength uint8) (*account.ECDSAAccount, error) {
	ecdsaAccount, err := account.CreateNewAccountWithMnemonic(language, strength, types.Nist)
...
func (xcc *XchainCryptoClient) CreateNewAccountAndSaveSecretKey(path string, language int, strength uint8, password string) (*account.ECDSAInfo, error) {
	ecdasaInfo, err := key.CreateAndSaveSecretKey(path, walletRand.SimplifiedChinese, account.StrengthHard, password, types.Nist)
...
func (xcc *XchainCryptoClient) ExportNewAccountWithMnemonic(path string, language int, strength uint8) error {
	err := account.ExportNewAccountWithMnemonic(path, language, strength, types.Nist)
	return err
}
...
func (xcc *XchainCryptoClient) EncryptByAESKey(info string, cypherKey string) (string, error) {
	cipherInfo, err := aes.Encrypt([]byte(info), []byte(cypherKey))
	if err != nil {
		return "", err
	}
...
func (xcc *XchainCryptoClient) DecryptByAESKey(cipherInfo string, cypherKey string) (string, error) {
	info, err := aes.Decrypt([]byte(cipherInfo), []byte(cypherKey))
	if err != nil {
		return "", err
	}
...
func (xcc *XchainCryptoClient) DecryptByHdKey(publicKey, privateAncestorKey, cypherText string) (string, error) {
	return walletBase.Decrypt(publicKey, privateAncestorKey, cypherText)
}
...
func (xcc *XchainCryptoClient) SecretSplit(totalShareNumber, minimumShareNumber int, secret []byte) (shares map[int]*big.Int, err error) {
	curve := elliptic.P256()
	return complexSecret.ComplexSecretSplit2(totalShareNumber, minimumShareNumber, secret, curve)
}

// 通过收集到的碎片来还原出秘密
func (xcc *XchainCryptoClient) SecretRetrieve(shares map[int]*big.Int) ([]byte, error) {
	curve := elliptic.P256()
	return complexSecret.ComplexSecretRetrieve2(shares, curve)
}
...
func (xcc *XchainCryptoClient) ZkpProveMiMC(ccs frontend.CompiledConstraintSystem, pk groth16.ProvingKey, secret []byte) (groth16.Proof, error) {
	return mimc.Prove(ccs, pk, secret)
}

func (xcc *XchainCryptoClient) ZkpVerifyMiMC(proof groth16.Proof, vk groth16.VerifyingKey, hashResult []byte) (bool, error) {
