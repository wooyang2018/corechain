...
func GetBinaryEcdsaPrivateKeyFromFile(path string, password string) ([]byte, error) {
	...
	originalContent, err := aes.Decrypt(content, newPassword)
	if err != nil {
...
func GetBinaryEcdsaPrivateKeyFromString(encryptPrivateKey string, password string) ([]byte, error) {
	...
	originalContent, err := aes.Decrypt([]byte(encryptPrivateKey), newPassword)
	if err != nil {
...
func EncryptByKey(info string, key string) (string, error) {
	...
	// 加密info
	cipherInfo, err := aes.Encrypt([]byte(info), newPassword)
	if err != nil {
		return "", err
	}
...
func DecryptByKey(cipherInfo string, key string) (string, error) {
	...
	// 解密cipherInfo
	info, err := aes.Decrypt([]byte(cipherInfo), newPassword)
	if err != nil {
		return "", err
	}
