...
func EncryptAccount(info *account.ECDSAAccount, password string) (*account.ECDSAAccountToCloud, error) {
	...
	// 加密私钥
	encryptedPrivateKey, err := aes.Encrypt([]byte(info.JsonPrivateKey), newPassword)
	if err != nil {
		return nil, err
	}
	...
	// 加密助记词
	if info.Mnemonic != "" {
		encryptedMnemonic, err := aes.Encrypt([]byte(info.Mnemonic), newPassword)
		if err != nil {
			return nil, err
		}
...
func savePrivateKey(path string, password string, ecdsaAccount *account.ECDSAAccount) error {
	...
	// 加密密钥文件
	encryptContent, err := aes.Encrypt([]byte(ecdsaAccount.JsonPrivateKey), newPassword)
	if err != nil {
