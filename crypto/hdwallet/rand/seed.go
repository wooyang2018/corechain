...
import (
	...
	"github.com/wooyang2018/corechain/crypto/hdwallet/wordlist"
...
func GenerateMnemonic(entropy []byte, language int) (string, error) {
	...
	for i := sentenceLength - 1; i >= 0; i-- {
		...
		// 把11个比特补齐为 2个字节
		wordBytes := utils.BytesPad(word.Bytes(), 2)
...
func GetEntropyFromMnemonic(mnemonic string, language int) ([]byte, error) {
	...
	entropyBytes := utils.BytesPad(entropy.Bytes(), entropyByteSize)
	entropyWithChecksumBytes := utils.BytesPad(b.Bytes(), fullByteSize)

	// 检查校验位是否正确
	newEntropyWithChecksumBytes := utils.BytesPad(addChecksum(entropyBytes), fullByteSize)
	if !utils.BytesCompare(entropyWithChecksumBytes, newEntropyWithChecksumBytes) {
...
func GetEntropyFromOldMnemonic(mnemonic string, language int) ([]byte, error) {
	...
	// 检查校验位是否正确
	newEntropyWithChecksumBytes := utils.BytesPad(addOldChecksum(entropyBytes), fullByteSize)
	if !utils.BytesCompare(entropyWithChecksumBytes, newEntropyWithChecksumBytes) {
		return nil, fmt.Errorf("The checksum within the Mnemonic sentence incorrect fake:%v - real:%v, entropy:%v, mnemonic:%v.", entropyWithChecksumBytes, newEntropyWithChecksumBytes, entropyBytes, mnemonic)
	}
...
func getWordsFromMnemonicSentence(mnemonic string) ([]string, error) {
	...
	if !utils.StringInSlice(strconv.Itoa(numOfWords), validNumSlice) {
...
func checkWordsWithinLanguageWordList(words []string, wordList []string) error {
	...
	for i := 0; i < numOfWords; i++ {
		if !utils.StringInSlice(words[i], wordList) {
			// 助记词不合法，单词未被支持
