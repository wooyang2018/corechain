...
func VerifyMultiSig(keys []*ecdsa.PublicKey, sign []byte, message []byte) (bool, error) {
	if len(keys) < MinimumParticipant {
		return false, TooSmallNumOfkeysError
	}

	//	sig, err := unmarshalMultiSignature(signature)
	sig := new(types.MultiSignature)
	err := json.Unmarshal(sign, sig)
	if err != nil {
