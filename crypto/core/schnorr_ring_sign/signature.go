...
//
//    QC order to close the ring, or say fulfill the gap, someone must have a private key which
...
//
// 6. Now we get everything, the ring is closed.
//    The Output signature: (P(0), ..., P(R-1), e(0), s(0), ..., s(R-1))
//
...
func Sign(keys []*ecdsa.PublicKey, privateKey *ecdsa.PrivateKey, message []byte) ([]byte, error) {
	...
	seed, err := walletRand.GenerateSeedWithStrengthAndKeyLen(walletRand.KeyStrengthHard, walletRand.KeyLengthInt64)
	if err != nil {
		return nil, err
	}
	...
	log.Printf("signerIndex is: %d", signerIndex)
	...
	k, err := walletRand.GenerateSeedWithStrengthAndKeyLen(walletRand.KeyStrengthHard, walletRand.KeyLengthInt32)
	if err != nil {
		return nil, err
	}
	...
	for i := (signerIndex + 1) % lenOfRing; i != signerIndex; i = (i + 1) % lenOfRing {
		// Choose a random number s((r+1)%R), i.e. s(i) within [1:N-1]
		s, err := walletRand.GenerateSeedWithStrengthAndKeyLen(walletRand.KeyStrengthHard, walletRand.KeyLengthInt32)
		if err != nil {
			return nil, err
		}
	...
	sigRing := &types.RingSignature{}
	//	sigRing.Curve = curve
	sigRing.CurveName = curve.Params().Name
	keyFactors := make([]*types.PublicKeyFactor, len(keys))
	for index, key := range keys {
		//		keyFactor := &PublicKeyFactor{}
		keyFactor := new(types.PublicKeyFactor)
		keyFactor.X = key.X
		keyFactor.Y = key.Y
	...
	//	sigContent, err := asn1.Marshal(base.RingSignature{sigRing.Curve, sigRing.Members, sigRing.E, sigRing.S})
	if err != nil {
		return nil, err
	}

	// 组装超级签名
	xuperSig := &types.XuperSignature{
		SigType:    types.SchnorrRing,
...
func checkPublicKeysMatchSignature(keys []*ecdsa.PublicKey, signature *types.RingSignature) bool {
	// 判断是否传入的公钥数组的大小和环签名中的公钥数组的大小是一致的
	if len(keys) != len(signature.Members) {
		log.Printf("key length and ring key length does not match")
		return false
	}

	// 把传入的公钥数组转为一个map，其key是公钥的X坐标，value是公钥的Y坐标
	publicKeyMap := make(map[string]string)
	for _, key := range keys {
		publicKeyMap[key.X.String()] = key.Y.String()
	}

	// 判断是否环签名中的公钥数组的每个公钥都在传入的公钥数组中存在
