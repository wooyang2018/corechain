...
func Sign(privateKey *ecdsa.PrivateKey, message []byte) (schnorrSignature []byte, err error) {
	...
	// 生成Schnorr签名：(sum(S), R)
	schnorrSig := &types.SchnorrSignature{
		E: intE,
		S: intS,
	}
...
// QC order to verify the signature, only need to check the equation:
...
func Verify(publicKey *ecdsa.PublicKey, sig []byte, message []byte) (valid bool, err error) {
	signature := new(types.SchnorrSignature)
