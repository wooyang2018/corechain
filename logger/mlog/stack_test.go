...
func TestCallerMidstackInlined(t *testing.T) {
	...
	if got, want := c.Frame().Function, "github.com/wooyang2018/corechain/logger/mlog.f3"; got != want {
...
func TestCallerPanic(t *testing.T) {
	...
	defer func() {
		if recover() != nil {
			...
			c1 := Caller(panicIdx + 1)
			if got, want := c1.Frame().Function, "github.com/wooyang2018/corechain/logger/mlog.TestCallerPanic"; got != want {
...
func TestTracePanic(t *testing.T) {
	...
	defer func() {
		if recover() != nil {
			...
			if got, want := trace[panicIdx+1].Frame().Function, "github.com/wooyang2018/corechain/logger/mlog.TestTracePanic"; got != want {
...
const importPath = "github.com/wooyang2018/corechain/logger/mlog"

type testType struct{}

func (tt testType) testMethod() (c Call, file string, line int, ok bool) {
	c = Caller(0)
	_, file, line, ok = runtime.Caller(0)
	line--
	return
...
func TestCallFormat(t *testing.T) {
	...
	data := []struct {
		...
		{c, "func", "%+n", "github.com/wooyang2018/corechain/logger/mlog.TestCallFormat"},
		{c, "func", "%k", "mlog"},
		{c, "func", "%+k", "github.com/wooyang2018/corechain/logger/mlog"},
		...
		{c2, "meth", "%+n", "github.com/wooyang2018/corechain/logger/mlog.testType.testMethod"},
		{c2, "meth", "%k", "mlog"},
		{c2, "meth", "%+k", "github.com/wooyang2018/corechain/logger/mlog"},
...
func getTrace(t *testing.T) (CallStack, int) {
	cs := StackTrace().TrimRuntime()
	_, _, line, ok := runtime.Caller(0)
	line--
	if !ok {
		t.Fatal("runtime.Caller(0) failed")
...
func deepStack(depth int, b *testing.B) CallStack {
	if depth > 0 {
...
func BenchmarkCallerAndVFmt(b *testing.B) {
	for i := 0; i < b.N; i++ {
		fmt.Fprint(ioutil.Discard, Caller(0))
	}
}

func BenchmarkTraceAndVFmt(b *testing.B) {
	for i := 0; i < b.N; i++ {
		fmt.Fprint(ioutil.Discard, StackTrace())
	}
}
