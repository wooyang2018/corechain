...
func (p *P2PServerV1) Init(ctx *netBase.NetCtx) error {
	...
	_, _, err = mnet.DialArgs(p.address)
	if err != nil {
		log.Printf("network address error: %v", err)
		return ErrAddressIllegal
	}
...
func (p *P2PServerV1) serve() {
	...
	if p.config.IsTls {
		creds, err := network.NewTLS(p.config.KeyPath, p.config.ServiceName)
		if err != nil {
			panic(err)
		}
	...
	network, ip, err := mnet.DialArgs(p.address)
	if err != nil {
	...
	protos.RegisterP2PServiceServer(server, p)
...
func (p *P2PServerV1) PeerInfo() protos.PeerInfo {
	_, ip, err := mnet.DialArgs(p.address)
	if err != nil {
...
func (p *P2PServerV1) connectBootNodes() {
	...
	for _, peerInfo := range remotePeerInfos {
		...
		p.log.Debug("connect boot node", "local", p.address, "peer", peerInfo.Address, "account", peerInfo.Account)
	}

	p.log.Debug("connect boot node", "local", p.address, "send", len(addresses), "recv", len(remotePeerInfos), "dynamic", len(p.dynamicNodes))
	return
}
...
func (p *P2PServerV1) connectStaticNodes() {
	...
	for _, addresses := range p.staticNodes {
		for _, address := range addresses {
			...
			_, err := p.pool.Get(address)
			if err != nil {
				p.log.Warn("network connect to peer failed", "address", address, "error", err)
				continue
			}
