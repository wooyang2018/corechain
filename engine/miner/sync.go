...
import (
	...
	"github.com/wooyang2018/corechain/consensus/single"
...
func (t *Miner) getValidators(excludeAddr string) ([]string, error) {
	...
	var info single.ValidatorsInfo
...
func (t *Miner) getMaxBlockHeight(ctx xctx.Context) (string, int64, []byte, error) {
	...
	for _, response := range responses {
		var status protos.ChainStatus
		err = network.Unmarshal(response, &status)
		if err != nil {
...
func (t *Miner) syncWithValidators(ctx xctx.Context, timeout time.Duration) error {
...
func (t *Miner) syncWithLongestChain(ctx xctx.Context) (int, error) {
...
func (t *Miner) syncWithNeighbors(ctx xctx.Context) error {
	for {
		currentHeight := t.ctx.Ledger.GetMeta().TrunkHeight
		height := currentHeight + 1
...
func (t *Miner) syncBlockWithHeight(ctx xctx.Context, height int64, size int) (int, error) {
...
func (t *Miner) getBlocksByHeight(ctx xctx.Context, height int64, size int) ([]*protos.InternalBlock, error) {
...
func (t *Miner) fillBlockTxs(ctx xctx.Context, block *protos.InternalBlock) error {
	trace := traceSync()
	txids := block.GetMerkleTree()[:block.GetTxCount()]

	blockTxs := make([]*protos.Transaction, len(txids))
...
func (t *Miner) downloadMissingTxs(ctx xctx.Context, blockid []byte, txidx []int32) ([]*protos.Transaction, error) {
	if len(txidx) == 0 {
		return nil, nil
	}
	...
	for _, response := range responses {
		...
		var block protos.GetBlockTxsResponse
...
// 追加区块到账本中
func (t *Miner) batchConfirmBlocks(ctx xctx.Context, blocks []*protos.InternalBlock) error {
	if len(blocks) < 1 {
		return nil
	}
	...
	ctx.GetLog().Debug("batch confirm block to ledger succ", "blockCount", len(blocks))
	return nil
}
...
func (q *quorumBlockCounter) updateBlock(blk *protos.InternalBlock) {
	for i := range q.count {
...
func (q *quorumBlockCounter) quorumBlock() *protos.InternalBlock {
	if len(q.count) == 0 {
		return nil
	}
...
func quorumBlocks(responses []*protos.CoreMessage, blockAmount int) []*protos.InternalBlock {
	var peerBlocks [][]*protos.InternalBlock
	for _, response := range responses {
		if response.GetHeader().GetErrorType() != protos.CoreMessage_SUCCESS {
			continue
		}
		var block protos.GetBlockHeaderResponse
		err := network.Unmarshal(response, &block)
		if err != nil {
			continue
		}
	...
	var retBlocks []*protos.InternalBlock
...
func (t *Miner) findForkPoint(ctx xctx.Context) (*protos.InternalBlock, error) {
	currentHeight := t.ctx.Ledger.GetMeta().TrunkHeight
	ledger := t.ctx.Ledger
...
func (m *Miner) handleFork(ctx xctx.Context) error {
