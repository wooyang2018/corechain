...
func NewMiner(ctx *base.ChainCtx) *Miner {
	obj := &Miner{
		ctx: ctx,
		log: ctx.GetLog(),
	}
...
// 处理P2P网络中接收到的区块
func (t *Miner) ProcBlock(ctx xctx.Context, block *protos.InternalBlock) error {
	return nil
}
...
func (t *Miner) step() error {
	...
	ctx.GetLog().Debug("miner step", "ledgerTipHeight", ledgerTipHeight, "ledgerTipId",
	...
	if t.status == statusFollowing {
		...
		ctx.GetLog().Debug("miner syncWithValidators before CompeteMaster", "originTipHeight", ledgerTipHeight,
	...
	trace("competeMaster")
	ctx.GetLog().Debug("compete master result", "height", ledgerTipHeight+1, "isMiner", isMiner, "isSync", isSync, "err", err)
	if err != nil {
		return err
	}
	...
	if isMiner {
		if t.status == statusFollowing || isSync {
			...
			if ledgerTipHeight != t.ctx.Ledger.GetMeta().TrunkHeight {
				ctx.GetLog().Debug("miner change follow=>miner", "originTipHeight", ledgerTipHeight, "currentLedgerHeight",
...
// mining 挖矿生产区块
func (t *Miner) mining(ctx xctx.Context) error {
	...
	ctx.GetLog().Debug("start confirm block for miner", "originalBlockId", utils.F(origBlkId),
...
// 裁剪掉账本最新的区块
func (t *Miner) truncateForMiner(ctx xctx.Context, target []byte) error {
...
func (t *Miner) packBlock(ctx xctx.Context, height int64,
	now time.Time, consData []byte) (*protos.InternalBlock, error) {
	// 区块大小限制
	...
	txList := make([]*protos.Transaction, 0, len(generalTxList)+1+1)
	// 先coinbase tx
...
func (t *Miner) getTimerTx(height int64) (*protos.Transaction, error) {
	autoTx, err := t.ctx.State.GetTimerTx(height)
	if err != nil {
		t.log.Error("Get timer tx error", "error", err)
		return nil, base.ErrGenerateTimerTxFailed
	}

	return autoTx, nil
}

func (t *Miner) getUnconfirmedTx(sizeLimit int) ([]*protos.Transaction, error) {
...
func (t *Miner) getAwardTx(height int64) (*protos.Transaction, error) {
...
func (t *Miner) confirmBlockForMiner(ctx xctx.Context, block *protos.InternalBlock) error {
	...
	if confirmStatus.Succ {
		if confirmStatus.Orphan {
			ctx.GetLog().Debug("the mined blocked was attached to branch,no need to play",
				"blockId", utils.F(block.Blockid))
			return nil
		}
		ctx.GetLog().Debug("ledger confirm block success", "height", block.Height,
			"blockId", utils.F(block.Blockid))
	} else {
	...
	ctx.GetLog().Debug("confirm block for miner succ", "blockId", utils.F(block.Blockid))
	return nil
}
